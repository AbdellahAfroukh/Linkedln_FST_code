...
import { User, UserSimple, Chat } from "@/types";
import { useAuth } from "@/features/auth/hooks/use-auth";

interface AddMembersDialogProps {
  chatId: number;
  currentMembers: UserSimple[];
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onMembersAdded: (chat: Chat) => void;
}
...
export default function AddMembersDialog({
  ...
  const loadUsers = async () => {
    try {
      ...
      const filtered = response.users.filter(
        (u) => u.id !== currentUser?.id && !currentMemberIds.includes(u.id),
      );
  ...
  const handleAddMembers = async () => {
    ...
    try {
      setIsAdding(true);
      const updatedChat = await chatsApi.addMembers(chatId, selectedMembers);

      toast({
        title: "Success",
        description: `Added ${selectedMembers.length} member(s) to the group`,
      });
